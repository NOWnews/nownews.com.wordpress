    ...
    if (!class_exists('wpCloud\StatelessMedia\Bootstrap')) {
        ...
        final class Bootstrap extends \UsabilityDynamics\WP\Bootstrap_Plugin {
            ...
            public function register_metaboxes() {
                add_meta_box(
                        'sm-attachment-metabox', __('Google Cloud Storage', ud_get_stateless_media()->domain), array($this, 'attachment_meta_box_callback'), 'attachment', 'side', 'low'
                );
            }
            ...
            public function the_content_filter($content) {
                ...
                if ($upload_data = wp_upload_dir()) {
                    ...
                    if (!empty($upload_data['baseurl']) && !empty($content)) {
                        ...
                        $content = preg_replace('/(href|src)=(\'|")(https?:\/\/' . str_replace('/', '\/', $baseurl) . ')\/(.+?)(' . $file_ext . ')(\'|")/i', '$1=$2' . $image_host . '/' . ($root_dir ? $root_dir : '') . '$4$5$6', $content);
                    }
                }
